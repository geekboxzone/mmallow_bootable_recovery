...
//    LOGI("pItem->name=%s\n", pItem->name);
    ...
    src_buf = (char *)malloc(COMPARE_BUFFER_SIZE);
    if (src_buf == 0) {
    ...
    dest_buf = (char *)malloc(COMPARE_BUFFER_SIZE);
    if (src_buf == 0) {
       LOGE("malloc memory error\n");
       close(fd_src);
        close(fd_dest);
...
Value* WriteLoaderFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (write_loader(itemname,partition,(long)offset) < 0 ) { 
	return NULL;
	}
    else
    	{
...
static char* PrintSha1(uint8_t* digest) {
    char* buffer = (char*)malloc(SHA_DIGEST_SIZE*2 + 1);
    int i;
